...
router.post('/index1', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var panNo = req.body.pan;
    ...
    const response = await axios.post('https://testapi.karza.in/v2/pan', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
      Cart.update({owner:userId},{ "$pull": { "items": { "title": 'PAN CARD VERIFICATION' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Pan_count = day.Pan_count;
      ...
      var pan_output = [];
      pan_output.push(response.data.result.name);
      localStorage.pan_name = pan_output;
      localStorage.pan_no = panNo;
        const user = await User.findOne({
          _id: userId
        });

        const newC = await Cart.removeFormItem(user._id, 'FORM1');
