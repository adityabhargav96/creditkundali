...
router.post('/index44', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var mobile = req.body.epfuanmobile;
    ...
    const data = {
      consent: 'Y',
      mobile: req.body.epfuanmobile
    }

    const response = await axios.post('https://testapi.karza.in/v2/uan-lookup', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
      Cart.update({owner:userId},{ "$pull": { "items": { "title": 'EPF UAN' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      System.findOne({company_branch:req.user._id},function(err,system){
        system.Epf_Uan_Lookup_count = system.Epf_Uan_Lookup_count + 1;
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Epf_Uan_Lookup_count = day.Epf_Uan_Lookup_count;
          ...
          day.date = wah;
          day.Epf_Uan_Lookup_count = day.Epf_Uan_Lookup_count + 1;
          day.save(function (err, day) {
              if (err) {
                  return next(err);
              }
      ...
      look.push(response.data.result.uan);

        localStorage.mobile = mobile;
        localStorage.employee = look;
        const user = await User.findOne({
          _id: userId
        });

        const newC = await Cart.removeFormItem(user._id, 'FORM44');
