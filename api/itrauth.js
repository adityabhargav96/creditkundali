...
router.post('/index32', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var pan = req.body.itrpan;
    var acknowledge = req.body.itracknowledge;
    ...
    const data = {
      consent: 'Y',
      pan:req.body.itrpan,
      ack:req.body.itracknowledge
    }

    const response = await axios.post('https://testapi.karza.in/v2/itr', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
       Cart.update({owner:userId},{ "$pull": { "items": { "title": 'ITR AUTHENTICATION' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      System.findOne({company_branch:req.user._id},function(err,system){
        system.Itr_Auth_count = system.Itr_Auth_count + 1;
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Itr_Auth_count = day.Itr_Auth_count;
          ...
          day.date = wah;
          day.Itr_Auth_count = day.Itr_Auth_count + 1;
          day.save(function (err, day) {
              if (err) {
                  return next(err);
              }
        ...
        validity.push(response.data.result.validity);

        localStorage.itr_pan=pan;
        localStorage.itr_ack=acknowledge;
        localStorage.itr_status=status;
        localStorage.itr_validity=validity;
        ...
        const newC = await Cart.removeFormItem(user._id, 'FORM32');
