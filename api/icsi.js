...
router.post('/index21', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var icsino = req.body.icsi;
    var cpnumber = req.body.cpnumber;
    ...
    const data = {
      consent: 'Y',
      membership_no:req.body.icsi,
      cp_no:''
    }

    const response = await axios.post('https://testapi.karza.in/v2/icsi', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
       Cart.update({owner:userId},{ "$pull": { "items": { "title": 'ICSI MEMBERSHIP' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      System.findOne({company_branch:req.user._id},function(err,system){
        system.Icsi_membership_count = system.Icsi_membership_count + 1;
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Icsi_membership_count = day.Icsi_membership_count;
          ...
          day.date = wah;
          day.Icsi_membership_count = day.Icsi_membership_count + 1;
          day.save(function (err, day) {
              if (err) {
                  return next(err);
              }
       ...
       result.push(response.data.result[i]);
        ...
        localStorage.icsi = result;
        localStorage.icsino = icsino;
        localStorage.cpnumber = cpnumber;
        ...
        const newC = await Cart.removeFormItem(user._id, 'FORM21');
