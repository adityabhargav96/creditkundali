...
router.post('/index18', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var reg = req.body.fssai;
    ...
    const data = {
      consent: 'Y',
      reg_no: req.body.fssai
    }

    const response = await axios.post('https://testapi.karza.in/v2/fssai', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
       Cart.update({owner:userId},{ "$pull": { "items": { "title": 'FSSAI LICENSE' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      System.findOne({company_branch:req.user._id},function(err,system){
        system.Fssai_license_count = system.Fssai_license_count + 1;
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Fssai_license_count = day.Fssai_license_count;
          ...
          day.date = wah;
          day.Fssai_license_count = day.Fssai_license_count + 1;
          day.save(function (err, day) {
              if (err) {
                  return next(err);
              }
      ...
      lictype.push(response.data.result.LicType);
      licno.push(response.data.result.LicNO);
      ...
      localStorage.address=address;
        ...
        const newC = await Cart.removeFormItem(user._id, 'FORM18');
