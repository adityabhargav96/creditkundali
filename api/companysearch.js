...
router.post('/index9', async (req, res, next) => {
  try {
    var userId = req.user._id;
    var compname = req.body.companyn;
    ...
    const data = {
      consent: 'Y',
      company_name: req.body.companyn
    }

    const response = await axios.post('https://testapi.karza.in/v2/compsearch', data, config);
    ...
    switch (response.data['status-code']) {
      ...
      case "103":
        ...
        break;
      case "400":
      Cart.update({owner:userId},{ "$pull": { "items": { "title": 'COMPANY SEARCH BY NAME' } }}, { safe: true, multi:true }, function(err, obj) {
      ...
      System.findOne({company_branch:req.user._id},function(err,system){
        system.Company_SearchByName_count = system.Company_SearchByName_count + 1;
      ...
      Day.findOne({company_branch:req.user._id},function(err,day){
        ...
        day.save(function (err, day) {
          ...
          Yesterday.findOne({company_branch:req.user._id},function(err,yesterday){
            yesterday.Company_SearchByName_count = day.Company_SearchByName_count;
          ...
          day.date = wah;
          day.Company_SearchByName_count = day.Company_SearchByName_count + 1;
          day.save(function (err, day) {
              if (err) {
                  return next(err);
              }
      ...
      mock.push(response.data.result.result[i]);
    }
      console.log(mock)
      localStorage.compname = compname;
      localStorage.mock = mock;
        const user = await User.findOne({
          _id: userId
        });

        const newC = await Cart.removeFormItem(user._id, 'FORM9');
